...
class ShippingFlowHelper {
    ...
    async getCartTotal(): Promise<string> {
        const totalElement = this.page
            .locator('[data-testid="cart-total"], .total, :text("Total")')
            .last();
    ...
    async getShippingCost(): Promise<string> {
        const shippingElement = this.page
            .locator(
                '[data-testid="shipping-cost"], :text("Shipping"), :text("Μεταφορικά")',
            )
            .last();
...
test.describe("Shipping Integration Flow", () => {
    test("C1: Complete shipping cost calculation and total update", async ({
        page,
    }) => {
    ...
    test("C1a: Shipping input validation and error handling", async ({
        page,
    }) => {
        const helper = new ShippingFlowHelper(page);
