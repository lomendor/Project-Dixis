...
test.describe("Mobile Navigation", () => {
    ...
    test("should show mobile menu button on mobile screens", async ({
        page,
    }) => {
    ...
    test("should show hamburger icon when menu is closed and X icon when menu is open", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);

        // Check hamburger icon is shown (three horizontal lines)
        const hamburgerIcon = page.locator(
            '[data-testid="mobile-menu-button"] svg path[d*="M4 6h16M4 12h16M4 18h16"]',
        );
        ...
        const closeIcon = page.locator(
            '[data-testid="mobile-menu-button"] svg path[d*="M6 18L18 6M6 6l12 12"]',
        );
    ...
    test("should navigate to products page when mobile Products link is clicked", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
    ...
    test("should show login and register links for unauthenticated users", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
        ...
        await expect(
            page.locator('[data-testid="mobile-nav-login"]'),
        ).toBeVisible();
        await expect(
            page.locator('[data-testid="mobile-nav-register"]'),
        ).toBeVisible();

        // Should not show cart or dashboard links
        await expect(
            page.locator('[data-testid="mobile-nav-cart"]'),
        ).not.toBeVisible();
        await expect(
    ...
    test("should show cart link for authenticated consumers", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
    ...
    test("should show dashboard link for authenticated producers", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
    ...
    test("should show user profile info in mobile menu when authenticated", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
        ...
        await expect(
            page.locator('[data-testid="mobile-logout-btn"]'),
        ).toBeVisible();
    });

    test("should logout user when mobile logout button is clicked", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
    ...
    test("should navigate to cart when mobile cart link is clicked", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
    ...
    test("should navigate to dashboard when mobile dashboard link is clicked", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
    ...
    test("should navigate to login when mobile login link is clicked", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
    ...
    test("should navigate to register when mobile register link is clicked", async ({
        page,
    }) => {
        const helper = new MobileNavHelper(page);
...
test.describe("Desktop Navigation", () => {
    ...
    test("should hide mobile menu button on desktop screens", async ({
        page,
    }) => {
    ...
    test("should show desktop navigation links", async ({ page }) => {
        ...
        await expect(
            page.locator('[data-testid="nav-register"]'),
        ).toBeVisible();
    });
});
