...
class PRevidenceCollector {
  ...
  private setupNetworkLogging() {
    ...
    this.page.on('request', (request) => {
      ...
      if (request.url().includes('/api/v1/shipping/quote')) {
        try {
          log.request = JSON.parse(request.postData() || '{}');
          console.log('🌐 Shipping API Request:', JSON.stringify(log, null, 2));
        } catch (e) {
          console.log('🌐 Shipping API Request (raw):', request.postData());
        }
      }
      ...
      if (logIndex !== -1) {
        try {
          ...
          if (response.url().includes('/api/v1/shipping/quote')) {
            ...
            await this.captureScreenshot('shipping-api-response', true);
          }
  ...
  async testShippingCostCalculation() {
    ...
    const postalField = this.page.locator('[data-testid="postal-code-input"], input[name="postal_code"], input[placeholder*="ΤΚ"]').first();
    const cityField = this.page.locator('[data-testid="city-input"], input[name="city"], input[placeholder*="city"]').first();
    ...
    await expect(this.page.locator('text=Athens Express')).toBeVisible({ timeout: 10000 });
    ...
    await this.page.waitForTimeout(2000);
    await expect(this.page.locator('text=Northern Courier')).toBeVisible({ timeout: 10000 });
  ...
  async verifyTotalRecalculation() {
    ...
    const totalElements = this.page.locator('[data-testid="total"], .total, text=/Total.*€/');
    const shippingElements = this.page.locator('[data-testid="shipping-cost"], text=/Shipping.*€/');
    
    // Capture current totals
    const totalCount = await totalElements.count();
    const shippingCount = await shippingElements.count();
    
    console.log(`Found ${totalCount} total elements, ${shippingCount} shipping elements`);
    ...
    if (shippingCount > 0) {
      for (let i = 0; i < shippingCount; i++) {
        const shippingText = await shippingElements.nth(i).textContent();
  ...
  async verifyOrderDetails() {
    ...
    await expect(this.page.locator('[data-testid="order-success"], text=/Order.*successful/i')).toBeVisible();
  ...
  generateEvidenceReport() {
    ...
    const orderRequests = this.networkLogs.filter(log => 
    ...
    if (shippingRequests.length > 0) {
      ...
      shippingRequests.forEach((req, index) => {
        ...
        if (req.request) {
          console.log('   Request payload:', JSON.stringify(req.request, null, 2));
        }
        if (req.response) {
      ...
      orderRequests.forEach((req, index) => {
        ...
        if (req.response) {
          ...
          if (req.response.data) {
            ...
            console.log(`   📍 Shipping Address: ${order.shipping_address?.city}, ${order.shipping_address?.zip}`);
            console.log(`   🚚 Shipping: ${order.shipping?.carrier} (${order.shipping?.cost}€, ${order.shipping?.eta_days} days)`);
          }
        }
      });
    }
    ...
    console.log('🔍 Check the console output above for API request/response details');
    console.log('='.repeat(60));
  }
}

test.describe('PR Evidence Collection - Shipping Integration', () => {
  test('PR #37 & #38: Complete shipping and checkout evidence', async ({ page }) => {
    ...
    console.log('📍 Servers: API http://127.0.0.1:8001 | Frontend http://localhost:3001');
    console.log('');
  ...
  test('Network capture verification - shipping API only', async ({ page }) => {
    const collector = new PRevidenceCollector(page);
    ...
    const testCases = [
      ...
      { postal: '54623', city: 'Thessaloniki', expected: 'Northern Courier' },
    ...
    for (const testCase of testCases) {
      ...
      const postalField = page.locator('[data-testid="postal-code-input"], input[name="postal_code"]').first();
      const cityField = page.locator('[data-testid="city-input"], input[name="city"]').first();
      ...
      await collector.captureScreenshot(`network-test-${testCase.city.toLowerCase()}`);
    }
