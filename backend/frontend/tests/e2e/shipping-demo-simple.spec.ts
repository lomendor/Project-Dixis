    ...
    test("capture cart page with shipping calculation", async ({ page }) => {
        ...
        if (foundFields.length > 0) {
            ...
            try {
                ...
                const cityField =
                    foundFields.length > 1
                        ? page.locator(foundFields[1]).first()
                        : null;
                ...
                console.log("⚠️ Could not fill shipping fields:", e instanceof Error ? e.message : String(e));
            }
        } else {
        ...
        console.log(
            `📊 Page has shipping-related content: ${hasShippingContent ? "Yes" : "No"}`,
        );
    ...
    test("verify page elements and structure", async ({ page }) => {
        ...
        for (const selector of elements) {
            try {
                ...
                console.log(`❌ Error checking ${selector}: ${e instanceof Error ? e.message : String(e)}`);
            }
        }
        ...
        for (let i = 0; i < Math.min(inputCount, 10); i++) {
            try {
                ...
                console.log(
                    `  Input ${i + 1}: name="${name}", placeholder="${placeholder}", type="${type}"`,
                );
            } catch (e) {
