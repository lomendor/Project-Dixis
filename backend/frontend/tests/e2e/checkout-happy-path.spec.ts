...
class CheckoutFlowHelper {
    ...
    async completeCartShipping() {
        ...
        const postalField = this.page.locator(
                ...
                try {
                    ...
                    if (responseBody.order || responseBody.data) {
                        ...
                        if (order.shipping_carrier) {
                            console.log(
                                `   Shipping: ${order.shipping_carrier} (€${order.shipping_cost}, ${order.shipping_eta_days} days)`,
                            );
                        }
                    }
                } catch (e) {
                    console.log(
                        "🛒 Order response (non-JSON):",
                        await response.text(),
                    );
                }
            }
...
test.describe("Checkout Happy Path", () => {
    test("C2: Complete checkout flow from cart to confirmation", async ({
        page,
    }) => {
        ...
        if (orderPayload) {
            ...
            console.log(
                "   Has shipping info:",
                !!orderPayload.shipping_address || !!orderPayload.postal_code,
            );
            console.log(
                "   Payment method:",
            ...
            expect(
                orderPayload.shipping_method || orderPayload.payment_method,
            ).toBeTruthy();
        }
    });

    test("C2a: Order confirmation displays shipping details", async ({
        page,
    }) => {
        const helper = new CheckoutFlowHelper(page);
        ...
        const shippingInfo = page.locator(
            ':text("Athens Express"), :text("Athens"), :text("11527")',
        );
    ...
    test("C2b: Multiple checkout attempts handle correctly", async ({
        page,
    }) => {
        const helper = new CheckoutFlowHelper(page);
    ...
    test("C2c: Checkout with different payment methods", async ({ page }) => {
        ...
        const paymentInfo = page.locator(
            ':text("Cash on Delivery"), :text("COD"), :text("Payment on delivery")',
        );
