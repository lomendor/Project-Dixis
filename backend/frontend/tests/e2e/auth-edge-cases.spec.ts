...
test("auth edge-cases - wrong password then correct password", async ({
    page,
}) => {
...
test("auth edge-cases - empty fields validation", async ({ page }) => {
    ...
    const emailError = page.locator(
        'input[name="email"] + .error, [data-testid="email-error"]',
    );
    const passwordError = page.locator(
        'input[name="password"] + .error, [data-testid="password-error"]',
    );
...
test("auth edge-cases - invalid email format", async ({ page }) => {
    ...
    const errorIndicator = page.locator(
        '[data-testid="error-toast"], input[name="email"]:invalid',
    );
