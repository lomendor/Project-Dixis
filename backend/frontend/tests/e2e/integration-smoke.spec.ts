...
test.describe("Frontend ↔ API Integration Smoke Tests", () => {
    ...
    test("Core Flow: Login → View Products → Create Order", async ({
        page,
    }) => {
        ...
        const productsResponse = await page.request.get(
            `${API_BASE}/api/v1/public/products?per_page=2`,
        );
        ...
        console.log(
            `Using products: ${product1.name} (${product1.price}€) and ${product2.name} (${product2.price}€)`,
        );
        ...
        console.log(
            `✅ Order created successfully: €${totalAmount} total (€${actualSubtotal} subtotal, expected: €${expectedSubtotal})`,
        );
        ...
        console.log(
            `✅ Frontend order URL handled: ${pageTitle || "Page loaded"}`,
        );
    ...
    test("Products API Filtering and Search", async ({ page }) => {
        ...
        const searchResponse = await page.request.get(
            `${API_BASE}/api/v1/public/products?search=tomato`,
        );
        ...
        const categoriesResponse = await page.request.get(
            `${API_BASE}/api/v1/public/products`,
        );
        ...
        if (allProducts.data.length > 0) {
            ...
            if (firstProduct.categories && firstProduct.categories.length > 0) {
                ...
                const categoryResponse = await page.request.get(
                    `${API_BASE}/api/v1/public/products?category=${category}`,
                );
    ...
    test("Authentication States and Protected Routes", async ({ page }) => {
        ...
        console.log(
            `✅ Protected routes require authentication (status: ${protectedResponse.status()})`,
        );
        ...
        expect(profileData.user).toHaveProperty(
            "email",
