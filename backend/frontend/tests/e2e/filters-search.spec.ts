...
test("filters and search - apply search filter", async ({ page }) => {
    ...
    const searchInput = page
        .locator(
            'input[type="search"], input[placeholder*="search" i], input[name*="search" i], [data-testid="search-input"]',
        )
        .first();
    ...
    if (await searchInput.isVisible({ timeout: 3000 })) {
        ...
        if (filteredProductCount > 0) {
            expect(filteredProductCount).toBeLessThanOrEqual(
                initialProductCount,
            );

            // Verify search term appears in visible products
            const productTitles = page.locator(
                '[data-testid="product-title"], [data-testid="product-card"] h3',
            );
            ...
            const noResultsMessage = page.locator(
                '[data-testid="no-results"], .no-results, :has-text("no products found")',
            );
        ...
        const restoredProductCount = await page
            .locator('[data-testid="product-card"]')
            .count();
        expect(restoredProductCount).toBeGreaterThanOrEqual(
            initialProductCount,
        );
    } else {
...
test("filters and search - category filter", async ({ page }) => {
    ...
    const categoryFilter = page
        .locator(
            'select[name*="category" i], [data-testid="category-filter"], .category-filter select, .filter-category select',
        )
        .first();
    const categoryButtons = page.locator(
        '[data-testid="category-button"], .category-btn, .filter-btn[data-category]',
    );
    ...
    if (await categoryFilter.isVisible({ timeout: 3000 })) {
        ...
        const allButton = page
            .locator(
                '[data-testid="category-button"]:has-text("All"), .category-btn:has-text("All"), .filter-btn:has-text("All")',
            )
            .first();
...
test("filters and search - sort functionality", async ({ page }) => {
    ...
    const sortDropdown = page
        .locator(
            'select[name*="sort" i], [data-testid="sort-select"], .sort-filter select, select:has(option:has-text("price"))',
        )
        .first();
    ...
    if (await sortDropdown.isVisible({ timeout: 3000 })) {
        ...
        const initialPrices = await page
            .locator('[data-testid="product-price"], .price, .product-price')
            .allTextContents();
            ...
            for (const option of options) {
                ...
                if (
                    ...
                    (text.toLowerCase().includes("price") ||
                        text.toLowerCase().includes("τιμή"))
                ) {
