...
class ShippingIntegrationHelper {
    ...
    async navigateToCart() {
        ...
        await expect(
            this.page.locator('[data-testid="cart-item"]'),
    ...
    async enterShippingDetails(postalCode: string, city: string) {
        ...
        const postalCodeInput = this.page.locator(
            'input[name="postal_code"], input[placeholder*="postal"], input[placeholder*="ΤΚ"], input[id*="postal"], input[data-testid="postal-code"]',
        );
        await expect(postalCodeInput).toBeVisible({ timeout: 10000 });

        // Look for city input field
        const cityInput = this.page.locator(
            'input[name="city"], input[placeholder*="city"], input[placeholder*="πόλη"], input[id*="city"], input[data-testid="city"]',
        );
    ...
    async verifyShippingCalculation() {
        ...
        const shippingElements = [
            ...
            this.page.locator(
                ".shipping-info, .shipping-cost, .shipping-method",
            ),
        ];
...
test.describe("Shipping Integration Demo", () => {
    test("demonstrate shipping cost calculation on cart page", async ({
        page,
    }) => {
        ...
        const { postalCodeInput, cityInput } =
            await helper.enterShippingDetails("11527", "Athens");
        ...
        console.log(
            `  - Shipping Info Found: ${foundShipping ? "✅" : "⚠️"} ${foundShipping ? "Success" : "Partial"}`,
        );
        console.log(
            `  - Total Info Found: ${foundTotal ? "✅" : "⚠️"} ${foundTotal ? "Success" : "Partial"}`,
        );
        console.log(`  - Screenshots: ✅ 7 captured`);

        if (!foundShipping) {
            console.log(
                "⚠️ Note: Shipping calculation elements may have different selectors than expected",
            );
        }

        if (!foundTotal) {
            console.log(
                "⚠️ Note: Total calculation elements may have different selectors than expected",
            );
        }
