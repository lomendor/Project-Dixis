...
const fmtEUR = new Intl.NumberFormat('el-GR', {
  style: 'currency',
  currency: 'EUR',
});
...
export function ProductCard({
  ...
  const displayPrice = hasDiscount
    ? fmtEUR.format(discountPriceCents / 100)
    : typeof priceCents === 'number'
      ? fmtEUR.format(priceCents / 100)
      : '—';
  ...
  const producerUrl =
    producerSlug || producerId
      ? `/producers/${producerSlug || producerId}`
      : null;
    ...
    >
      ...
      <Link href={productUrl} className="block relative">
        <div
          ...
          className={`relative aspect-[4/3] sm:aspect-square w-full bg-neutral-100 overflow-hidden${isOOS ? ' opacity-50 grayscale' : ''}`}
        >
          {hasImage ? (
            <Image
              src={image}
          ...
          <div className="absolute top-2 left-2 flex flex-col gap-1">
            {isOOS && (
              <span
                ...
                className="inline-flex items-center px-2 py-0.5 rounded text-[10px] sm:text-xs font-semibold bg-neutral-800 text-white"
              >
                Εξαντλήθηκε
              </span>
            )}
            {isSeasonal && (
              <span
                data-testid="badge-seasonal"
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] sm:text-xs font-semibold bg-amber-400 text-white"
              >
                Εποχιακό
              </span>
            )}
            {hasDiscount && (
              <span
                ...
                className="inline-flex items-center px-2 py-0.5 rounded text-[10px] sm:text-xs font-bold bg-red-500 text-white"
              >
                -
                {Math.round(
                  ((priceCents - discountPriceCents) / priceCents) * 100
                )}
                %
              </span>
            )}
            {cultivationType && cultivationType !== 'conventional' && (
              <span
                data-testid="badge-cultivation"
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] sm:text-xs font-semibold ${
                  cultivationType === 'organic_certified'
                    ? 'bg-green-600 text-white'
                    : cultivationType === 'organic_transitional'
                      ? 'bg-lime-600 text-white'
                      : cultivationType === 'biodynamic'
                        ? 'bg-purple-600 text-white'
                        : cultivationType === 'traditional_natural'
                          ? 'bg-amber-600 text-white'
                          : 'bg-neutral-600 text-white'
                }`}
              >
                <FarmingMethodIcon method={cultivationType} size={14} />
                {cultivationType === 'organic_certified'
                  ? 'Βιολογικό'
                  : cultivationType === 'organic_transitional'
                    ? 'Βιολογικό'
            ...
            {isB2bOnly && (
              <span
                data-testid="badge-b2b"
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] sm:text-xs font-semibold bg-blue-600 text-white"
              >
                Χονδρική
              </span>
            )}
      ...
      <div className="flex flex-col flex-1 p-3 sm:p-3.5">
        ...
        {!hideProducerLink &&
          (producerUrl ? (
            ...
            <span
              ...
              className="text-[11px] sm:text-xs text-neutral-400 truncate block mb-1"
            >
              {producer || 'Παραγωγός'}
            </span>
          ))}
        ...
        <Link href={productUrl} className="block mb-1.5">
          <h3
            ...
            className="text-[15px] sm:text-base font-medium text-neutral-800 line-clamp-2 leading-snug hover:text-primary transition-colors"
          >
            {title}
          </h3>
        </Link>
        ...
        <div className="mt-auto pt-2 flex items-center justify-between gap-2">
          <div className="flex flex-col">
            <span
              ...
              className={`text-base sm:text-lg font-bold ${hasDiscount ? 'text-red-600' : 'text-neutral-900'}`}
            >
              {displayPrice}
            </span>
            {originalPrice && (
          ...
          <div data-testid="product-card-add" className="flex-shrink-0">
            <AddToCartButton
              ...
              imageUrl={image || undefined}
              producerId={producerId ? String(producerId) : undefined}
              producerName={producer || undefined}
              stock={stock}
              compact
            />
          </div>
...
export function ProductCardSkeleton() {
  return (
    <div
      ...
      className="flex flex-col h-full bg-white rounded-xl border border-neutral-200/70 overflow-hidden animate-pulse"
    >
