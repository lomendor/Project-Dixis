...
export default function CartPage(){
  ...
  function updateQty(id:string, v:number){
    const next = lines.map(l=> l.productId===id? {...l, qty: Math.max(1, v)} : l);
    setLines(next);
  ...
  async function checkout(){
    ...
    if(!r.ok){ setErr(j?.error || 'Σφάλμα παραγγελίας'); return; }
  ...
  return (<main style={{padding:16}}>
    ...
    <section style={{marginTop:16, display:'grid', gap:8, maxWidth:520}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:8}}>
        ...
        <button className="btn btn-primary" onClick={checkout}>Ολοκλήρωση παραγγελίας</button>
      </div>
