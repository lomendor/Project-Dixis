...
export function calcTotals(i: TotalsInput): Totals {
  ...
  const method = i.shippingMethod || 'COURIER';
  const shipBase = Number(i.baseShipping ?? (method==='PICKUP' ? 0 : 3.5));
  const cod = Number(method==='COURIER_COD' ? (i.codFee ?? 2.0) : 0);
  const shipping = round2(shipBase);
  const codFee = round2(cod);
  const taxRate = Number(i.taxRate ?? 0);
  const taxable = subtotal; // απλή εκδοχή: φόρος επί του subtotal
