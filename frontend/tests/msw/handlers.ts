...
// Rich Greek marketplace mock data aligned with api-mocks.ts
const mockProducts = [
  {
    id: 1,
...
export const authHandlers = [
  ...
  // Auth profile endpoint (aligned with api-mocks.ts)
  ...
  // Orders endpoint (missing from original)
  http.get(`${API_BASE}/orders`, () => {
    return HttpResponse.json([])
  }),

  // Categories endpoint (missing from original) 
  http.get(`${API_BASE}/categories`, () => {
    return HttpResponse.json(mockCategories)
  }),

  // Checkout endpoint (missing from original)
...
// Export all handlers with comprehensive API coverage
