...
class ManualEvidenceCollector {
    ...
    this.page.on('request', (request) => {
      ...
      if (request.url().includes('/api/v1/shipping/quote')) {
        try {
          ...
          console.log('🌐 SHIPPING API REQUEST:', JSON.stringify(log.request, null, 2));
        } catch (e) {
      ...
      if (request.url().includes('/api/v1/orders') && request.method() === 'POST') {
        try {
          log.request = JSON.parse(request.postData() || '{}');
          console.log('🛒 ORDER CREATION REQUEST:', JSON.stringify(log.request, null, 2));
        } catch (e) {
          console.log('🛒 ORDER CREATION REQUEST (raw):', request.postData());
        }
      }
  ...
  async analyzeShippingInfo(location: string) {
    ...
    const patterns = ['Express', 'Courier', 'Logistics', '€', 'day', 'days', 'cost', 'shipping'];
    const foundText = [];
    ...
    if (foundText.length > 0) {
      console.log(`  📦 Shipping info found: ${[...new Set(foundText)].join(', ')}`);
    } else {
      console.log('  ⚠️ No shipping info text found');
    }
    
    // Also check for price/cost elements
    const priceElements = this.page.locator('*[class*="price"], *[class*="cost"], *[class*="total"]');
    ...
    for (let i = 0; i < Math.min(priceCount, 5); i++) {
      try {
        ...
        if (await element.isVisible()) {
          const text = await element.textContent();
          console.log(`  💰 Price element ${i + 1}: "${text?.trim()}"`);
        }
      } catch (e) {
  ...
  async attemptCheckout() {
    ...
    console.log(`Checkout button status: enabled=${isEnabled}, text="${buttonText?.trim()}"`);
  ...
  generateReport() {
    ...
    const orderCalls = this.networkLogs.filter(log => 
    ...
    if (shippingCalls.length > 0) {
      ...
      shippingCalls.forEach((call, index) => {
        ...
        if (call.request) {
          ...
          console.log(`      📦 Package: ${req.weight}kg, ${req.volume}m³`);
        }
        if (call.response) {
      ...
      orderCalls.forEach((call, index) => {
        ...
        if (call.response && call.response.data) {
          ...
          if (order.shipping_address) {
            console.log(`      📍 Address: ${order.shipping_address.city}, ${order.shipping_address.zip}`);
          }
          
          if (order.shipping) {
            console.log(`      🚚 Shipping: ${order.shipping.carrier} (€${order.shipping.cost})`);
            console.log(`      ⏱️  Delivery: ${order.shipping.eta_days} day(s)`);
          }
        }
      });
    }
    ...
    console.log('🔍 API data captured above shows real-time shipping integration');
    console.log('='.repeat(70));
  }
}

test.describe('Manual PR Evidence Collection', () => {
  test('Complete shipping and checkout evidence - adaptive approach', async ({ page }) => {
    ...
    try {
      ...
      const shippingWorked = await collector.demonstrateShippingIntegration();
