...
test.describe('Filters and Search @smoke', () => {
  ...
  /**
   * CI-FLAKE-FIX-E2E-01: Category filter test.
   * Replaced all waitForLoadState('networkidle') with element-based waits.
   * networkidle hangs indefinitely in CI when background polling/analytics are active.
   */
  test('filters and search - category filter', async ({ page }) => {
    // Navigate to products page
    await page.goto('/products');

    // Wait for initial products to load (element-based, not networkidle)
    await expect(page.locator('[data-testid="product-card"]').first()).toBeVisible({ timeout: 15000 });
    ...
    if (await categoryFilter.isVisible({ timeout: 3000 })) {
      ...
      if (options.length > 1) {
        ...
        if (firstOption) {
          ...
          // Wait for UI to update (element-based)
          await page.waitForTimeout(1000);
          await expect(page.locator('[data-testid="product-card"]').first()).toBeVisible({ timeout: 10000 }).catch(() => {});
      ...
      // Look for "All" / "Όλα" button to reset
      const allButton = page.locator('[data-testid="category-button"]:has-text("All"), [data-testid="category-button"]:has-text("Όλα"), .category-btn:has-text("All")').first();
  ...
  /**
   * CI-FLAKE-FIX-E2E-01: Sort functionality test.
   * Replaced all waitForLoadState('networkidle') with element-based waits.
   */
  test('filters and search - sort functionality', async ({ page }) => {
    ...
    // Wait for initial products to load (element-based)
    await expect(page.locator('[data-testid="product-card"]').first()).toBeVisible({ timeout: 15000 });
