...
**Scope**: Display shipping costs in checkout before order placement

### Problem:
In PROD-ACCEPTANCE-01 audit (G5), discovered that checkout page does NOT display shipping costs.
User adds items, goes to checkout, sees subtotal but NO shipping line.
API `POST /api/v1/public/shipping/quote` works correctly (returns zone-based pricing),
but checkout UI never calls it.

### Solution:
Call shipping quote API when user enters valid postal code (5 digits), display shipping cost in checkout summary.

### Changes Made:
1. **Updated /checkout page** (`frontend/src/app/(storefront)/checkout/page.tsx`):
   - Added `ShippingQuote` interface for typed shipping state
   - Added state: `shippingQuote`, `shippingLoading`, `postalCode`
   - Added `fetchShippingQuote` callback using `apiClient.getZoneShippingQuote()`
   - Modified postal code input to trigger fetch on valid 5-digit entry
   - Updated order summary to show:
     - "Μεταφορικά" line with cost or "Εισάγετε Τ.Κ." prompt
     - "Δωρεάν" with emerald styling when free_shipping=true
     - Zone name display (e.g., "Ζώνη: Αττική")
     - Total updated to include shipping cost

2. **Created E2E test** (`frontend/tests/e2e/checkout-shipping-display.spec.ts`):
   - 7 test scenarios covering: initial state, API fetch, loading state, free shipping, incomplete postal, API error, total calculation
   - Uses route mocking (`page.route()`) for reliable tests without backend dependency
   - Tests data-testids: `shipping-line`, `shipping-cost`, `shipping-loading`, `shipping-pending`, `shipping-zone`, `checkout-total`

### Impact:
- ✅ Users now see shipping cost BEFORE placing order
- ✅ Zone-based pricing displayed (Αττική, Νησιά, etc.)
- ✅ Free shipping threshold clearly shown as "Δωρεάν"
- ✅ Total updates dynamically as postal code is entered
- ✅ Graceful fallback if API fails (just hides shipping until order)
- ✅ Build passes with Next.js 15.5.9
- ✅ No backend changes required (uses existing `/api/v1/public/shipping/quote` endpoint)
