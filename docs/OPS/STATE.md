...
- **Pass 20 Cart localStorage Canonical Redirect Fix**: Fixed cart appearing empty when user navigates between www.dixis.gr and dixis.gr. Root cause: localStorage is origin-specific (www.dixis.gr ≠ dixis.gr), users adding items on one domain couldn't see them on the other. Solution: Added canonical host redirect in Next.js middleware (www.dixis.gr → dixis.gr, 301 permanent). Benefits: (1) Cart persists across all navigation, (2) SEO improvement (canonical domain), (3) No user disruption. Changes: `frontend/middleware.ts` (canonical redirect logic), `frontend/tests/e2e/cart-prod-regress.spec.ts` (3 E2E tests verifying redirect + cart persistence), `doc/research/cart-bug-root-cause.md` (comprehensive root cause analysis). PR #1846 merged 2025-12-22T23:44:01Z. PROD proof (2025-12-22 23:48 UTC): `curl -I https://www.dixis.gr/products/1` → HTTP 301, location: https://dixis.gr:3000/products/1 ✅ | `curl -I https://www.dixis.gr/cart` → HTTP 301, location: https://dixis.gr:3000/cart ✅ | `curl https://dixis.gr/products/1` → HTTP 200 ✅ | `curl https://dixis.gr/cart` → HTTP 200 ✅. Canonical redirect enforced, cart persistence fixed. (Closed: 2025-12-22)
