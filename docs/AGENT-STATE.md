...
**TEST-FIX-ADMIN-JWT (2026-06-11)** — admin-jwt-rework quarantine FIXED & removed: `adminJwtHeaders()` helper in tests/TestCase.php mints the HS256 admin JWT; 57 tests green again. Also fixed a real GDPR bug it exposed: `ProducerDeletionService::anonymizeUser()` used `update()` but `anonymized_at`/`email_verified_at` aren't in `User::$fillable` → silently dropped (now `forceFill`).
